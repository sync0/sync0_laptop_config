...
** Tangling 
   ...
   warnings, which makes it easier to track bugs.
   #+BEGIN_SRC emacs-lisp
     (defun tangle-init ()
...
(global-set-key (kbd "<f5>") 'counsel-recentf)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; These variables determine the scope of snipes (and the incremental highlighter):
  ...
  ;; To map : to a python function (but only in python-mode):
                ...
                ;;             (push '(?qt "blockquote") evil-snipe-aliases)
                ;;             (push '(?bg "begin") evil-snipe-aliases)))
...
** Solaire mode
#+BEGIN_SRC emacs-lisp
(use-package solaire-mode :ensure t :after doom-themes 
  :init ; stuff to do before requiring the package
  (progn
...
** Nlinum
#+BEGIN_SRC emacs-lisp
(use-package nlinum-mode :ensure t 
...
** Nlinum hl 
#+BEGIN_SRC emacs-lisp
(use-package nlinum-hl :ensure t :after nlinum-mode
:config
...
** Hl-line mode
#+BEGIN_SRC emacs-lisp
(use-package hl-line-mode :ensure t :after nlinum-mode
:config
