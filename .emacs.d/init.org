 ...
 #+BEGIN_SRC emacs-lisp
   ...
   (defun my-buffer-face-mode-fixed ()
     ...
     (if (> (display-pixel-width) 2000)
       ...
       ;; laptop font size
       (setq buffer-face-mode-face '(:family "Source Code Pro" :height 125)))
     (buffer-face-mode))
...
#+BEGIN_SRC emacs-lisp
      ...
      (defun sync0-latex-compile ()
        ...
        ;;         (progn
        ;;           (setq texcmd "latexmk -pdflatex='lualatex -file-line-error --shell-escape -synctex=1' -pdf %f")
        ;;           (setq org-latex-default-packages-alist
        ;;                 '(("" "fontspec" t)
        ;;                   ("" "hyperref" t)))))
        ...
        ;; (defun my-auto-tex-cmd (backend)
        ;;   "When exporting from .org with latex,
        ;;     automatically run pdflatex, xelatex, or lualatex as
        ;;     appropriate, using latexmk."
        ;;   (let ((texcmd))
        ;;     (setq texcmd "latexmk -pdf %f")
        ;;     (if (string-match "LATEX_COMPILER: pdflatex" (buffer-string))
        ;;         (progn
        ;;           (setq texcmd "latexmk -pdflatex='pdflatex -file-line-error --shell-escape -synctex=1' -pdf %f")))
        ;;     (if (string-match "LATEX_COMPILER: lualatex" (buffer-string))
        ;;         (progn
        ;;           (setq texcmd "latexmk -pdflatex='lualatex -file-line-error --shell-escape -synctex=1' -pdf %f")))
        ;;     (if (string-match "LATEX_COMPILER: xelatex" (buffer-string))
        ;;         (progn
        ;;           (setq texcmd "latexmk -pdflatex='xelatex -file-line-error --shell-escape -synctex=1' -pdf %f")))

        ;;     (setq org-latex-pdf-process (list texcmd))))

  (setq org-latex-pdf-process (list "latexmk -pdflatex='lualatex -file-line-error --shell-escape -synctex=1' -pdf %f"))
