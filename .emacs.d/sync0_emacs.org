...
(setq sync0-zettelkasten-directory-sans (concat (getenv "HOME") "/Dropbox/org"))
...
(defun sync0-log-today-timestamp () 
  ...
  (add-hook 'before-save-hook (lambda ()
                                (when (and (eq major-mode 'org-mode)
                                           (or (equal default-directory (concat (getenv "HOME") "/Dropbox/org/"))
                                               ...
                                               (equal default-directory (concat (getenv "HOME") "/Dropbox/org/annotations/"))
  ...
  (defun sync0-zettelkasten-annotations-update-author ()
    ...
    (interactive)
 (when (equal default-directory sync0-zettelkasten-directory)
    (save-excursion
      (goto-char (point-min))
